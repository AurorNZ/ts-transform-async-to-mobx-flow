...
runTests('TypeScript-4');
runTests('TypeScript-5');

function runTests(tsVersion: 'TypeScript-4' | 'TypeScript-5') {
  let createTransformer: any;
  let ts: typeof import('typescript');

  describe(`Typescript version: ${tsVersion}`, () => {
    beforeAll(() => {
      ts =
        tsVersion === 'TypeScript-5'
          ? jest.requireActual('typescript')
          : jest.requireActual('typescript4');
      jest.doMock('typescript', () => ts);
      jest.resetModules();
      createTransformer = require('../src').default;
    });
